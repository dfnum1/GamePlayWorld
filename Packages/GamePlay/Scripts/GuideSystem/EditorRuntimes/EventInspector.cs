...
namespace Framework.Guide.Editor
{
    public partial class EventInspector
    {
        ...
        List<IUserData> m_vEvents = null;
        //------------------------------------------------------
        public void Open(List<IUserData> vNode, float fDuration = 0.1f )
        {
            m_vEvents = vNode;
        ...
        private void OnGUI(Rect rc)
        {
            ...
            if (GuideSystemEditor.EventPopMethod != null)
            {
                System.Type paramType = GuideSystemEditor.EventPopMethod.ReturnType;
                if (paramType.IsEnum)
                {
                    System.Enum evtType = (System.Enum)System.Enum.ToObject(paramType, m_nAddEventType);
                    GUILayout.BeginHorizontal();
                    object retType = GuideSystemEditor.EventPopMethod.Invoke(null, new object[] { evtType, "事件类型" });

                    m_nAddEventType = Convert.ToInt32(retType);
                    if (GuideSystemEditor.EventBuildByTypeMethod != null && GUILayout.Button("+", new GUILayoutOption[] { GUILayout.Width(20) }))
                    {
                        object ret = GuideSystemEditor.EventBuildByTypeMethod.Invoke(null, new object[] { System.Enum.ToObject(paramType, m_nAddEventType) });
                        if (ret != null && ret is IUserData)
                        {
                            m_vEvents.Add(ret as IUserData);
                        }
                    }
            ...
            if (GuideSystemEditor.EventDrawMethod != null)
            {
                for(int i=0; i < m_vEvents.Count; ++i)
                {
                    System.Reflection.MethodInfo pMethod = m_vEvents[i].GetType().GetMethod("GetEventType");
                    string strTypeName = "";
                    if (pMethod != null && GuideSystemEditor.EventNameGet != null)
                    {
                        object temp = GuideSystemEditor.EventNameGet.Invoke(null, new object[] { pMethod.Invoke(m_vEvents[i], new object[] { }) });
                        if(temp!=null)
                    ...
                    GUILayout.EndHorizontal();
                    m_vEvents[i] = GuideSystemEditor.EventDrawMethod.Invoke(null, new object[] { m_vEvents[i], null }) as IUserData;
