...
namespace Framework.Guide.Editor
{
    public partial class GuideEditorLogic
    {
        ...
        private Func<bool> isDocked
        {
            get
            {
                if (_isDocked == null)
                {
                    ...
                    MethodInfo isDockedMethod = typeof(GuideSystemEditor).GetProperty("docked", fullBinding).GetGetMethod(true);
        ...
        //------------------------------------------------------
        public GuideGroup GetCurGroup()
        {
            return m_pGroupData;
        }
        //------------------------------------------------------
        public void OnEnable(GuideSystemEditor pEditor)
        {
            m_pEditor = pEditor;
        ...
        public void OnGUI()
        {
            ...
            if (m_pGroupData != null)
            {
                ...
                if(ms_bDrawInfo)
                {
                    ...
                    EditorGUI.DrawRect(infoRect, new Color(0.25f, 0.25f, 0.25f, 0.7f));
                    GUILayout.BeginArea(infoRect);
                    float label = EditorGUIUtility.labelWidth;
                    {
                        GUILayout.BeginHorizontal();
                        EditorGUIUtility.labelWidth = 50;
                        ushort tag = (ushort)EditorGUILayout.DelayedIntField("Tag", m_pGroupData.Tag);
                        if(m_pGroupData.Tag != tag)
                        {
                            //! 检测是否已经被用
                            if(m_pEditor.CanUseTag(tag))
                            {
...

                    {
                        ...
                        EditorGUIUtility.labelWidth = label;
                        EditorGUILayout.HelpBox("若勾选结束后移除，则在触发结束后，本次运行游戏内不再执行触发检测。", MessageType.Info);
                        GUILayout.EndArea();
                    }
                }
                else
                {
                    Rect infoRect = new Rect(m_pEditor.position.width - 20, topPadding * zoom + 5, 20, 20);
                    if (GUI.Button(infoRect, EditorGUIUtility.TrIconContent("StepLeftButton").image))
                    {
                        ms_bDrawInfo = true;
                    }
                }
        ...
        public bool HasStepNodeWidgetGuide(int guide)
        {
            foreach (var db in m_vActioNodes)
            {
                ...
                {
                    var step = db.Value.bindNode as StepNode;
                    GuideSystemEditor.NodeAttr nodeAttr;
                    if (GuideSystemEditor.StepTypes.TryGetValue(step.type, out nodeAttr))
                    {
                        var ports = step.GetArgvPorts();
                        if (nodeAttr.argvs.Count == ports.Count)
                        {
                            for (int j = 0; j < nodeAttr.argvs.Count; ++j)
                            {
                                if (nodeAttr.argvs[j].attr.displayType == typeof(GuideGuid))
                                {
                                    ...
                                    if (field != null)
                                    {
                                        if (guide.ToString().CompareTo(field.GetValue(ports[j]).ToString()) == 0)
                                            return true;
                                    }
                                }
                            }
        ...
        //------------------------------------------------------
        public bool HasTriggerNode()
        {
            foreach (var db in m_vActioNodes)
            {
                if (db.Value.bindNode is TriggerNode)
                    return true;
            }
            return false;
        }
        ...
        //------------------------------------------------------
        public void OpenInspector(List<IUserData> vEvents)
        {
        ...
        public void Save(bool bSaveAs = false)
        {
            ...
            if (m_pGroupData == null)
            {
                GuideSystemEditor.Instance.ShowNotification(new GUIContent("m_pGroupData = null,保存失败"));
                return;
            }
                ...
                case EventType.KeyDown:
                    ...
                    else if (e.keyCode == KeyCode.Escape)
                    {
                        ...
                        if(!GuideSystem.getInstance().bDoing)
                        {
                            ...
                            if(guidePanel!=null) guidePanel.Hide();
                        }
                    }
        ...
        public void TestGuide(BaseNode pStartNode = null)
        {
            if (m_pGroupData == null)
            {
                GuideSystemEditor.Instance.ShowNotification(new GUIContent("m_pGroupData = null,测试失败"));
                return;
            }
        ...
        {
            GuideGroup pGroup = GuideSystemEditor.Instance.NewGuide();
        ...
        //------------------------------------------------------
        public GraphNode CreateExcudeNode(GuideSystemEditor.ExcudeParam param)
        {
            if (m_pGroupData == null) return null;
        ...
        //------------------------------------------------------
        public GraphNode CreateStepNode(GuideSystemEditor.StepParam param)
        {
            if (m_pGroupData == null) return null;
        ...
        //------------------------------------------------------
        public GraphNode CreateOpNode(GuideSystemEditor.OpParam param)
        {
            if (m_pGroupData == null) return null;
        ...
        public void RemoveSelectedNodes()
        {
            ...
            else if(m_pGroupData!=null)
            {
                ...
                if (!string.IsNullOrEmpty(m_pGroupData.strFile))
                {
                    string path = m_pGroupData.strFile.Replace("\\", "/");
