...
namespace Framework.Plugin
{
    ...
    public class PolygonTriangulation
    {
        ...
        public static bool IsContains(Vector3 center, List<Vector3> points, Vector3 p)
        {
            int plyCnt = points.Count;
            int crossNum = 0;
            for (int i = 0; i < plyCnt; ++i)
            {
                Vector3 v1 = points[i] + center;
                Vector3 v2 = points[(i + 1) % plyCnt] + center;
        ...
        public static int calcAreaOfPolygon2D(List<Vector3> verts)
        {
        ...
        public static List<int> ConvexTriangleIndex(List<Vector3> verts, List<int> indexes)
        {
            ...
            ConvexTriangleIndex(verts, indexes, ref triangles);
            return triangles;
        }
        public static void ConvexTriangleIndex(List<Vector3> verts, List<int> indexes, ref List<int> resultTris)
        {
            int len = verts.Count;
            //若是闭环去除最后一点
            ...
            {
                resultTris.Add(indexes[0]);
                resultTris.Add(indexes[i + 1]);
                resultTris.Add(indexes[i + 2]);
            }
        }

        public static bool WidelyTriangleIndex(List<Vector3> verts, List<int> indexs, ref List<int> resultTris)
        {
            int len = verts.Count;
            if (len <= 3)
            {
                ConvexTriangleIndex(verts, indexs, ref resultTris);
                return true;
            }
            ...
            resultTris.Add(indexs[prev]);
            resultTris.Add(indexs[canFragementIndex]);
            resultTris.Add(indexs[next]);
            //剔除可划分顶点及索引
            verts.RemoveAt(canFragementIndex);
            indexs.RemoveAt(canFragementIndex);

            //递归划分
            WidelyTriangleIndex(verts, indexs,ref resultTris);
        ...
        public static List<int> WidelyTriangleIndex(List<Vector3> verts, List<int> indexes)
        {
            ...
            int len = verts.Count;
            if (len < 3) return tTriangles;
            WidelyTriangleIndex(verts, indexes, ref tTriangles);
            return tTriangles;
        }
