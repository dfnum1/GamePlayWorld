...
namespace Framework.ED
{
    public class UIDrawUtils
    {
        ...
        //------------------------------------------------------
        private void DrawThickLineSegment(Vector3 start, Vector3 end, float width, Color color)
        {

            Vector3 perpendicular = Vector3.Cross(end, start).normalized * width * 0.5f;
        ...
        //------------------------------------------------------
        public static void DrawTextureNineSlice(Texture2D tex, Rect rect, int border)
        {
            if (tex == null) return;
            int w = tex.width;
            int h = tex.height;
            ...
            GUI.DrawTextureWithTexCoords(
                new Rect(rect.x, rect.y, border, border),
                tex,
                new Rect(0f, 1f - (float)border / h, (float)border / w, (float)border / h)
            );
            // 上
            GUI.DrawTextureWithTexCoords(
                new Rect(rect.x + border, rect.y, rect.width - 2 * border, border),
                tex,
                new Rect((float)border / w, 1f - (float)border / h, 1f - 2f * border / w, (float)border / h)
            );
            // 右上
            GUI.DrawTextureWithTexCoords(
                new Rect(rect.x + rect.width - border, rect.y, border, border),
                tex,
                new Rect(1f - (float)border / w, 1f - (float)border / h, (float)border / w, (float)border / h)
            );
            // 左
            GUI.DrawTextureWithTexCoords(
                new Rect(rect.x, rect.y + border, border, rect.height - 2 * border),
                tex,
                new Rect(0f, (float)border / h, (float)border / w, 1f - 2f * border / h)
            );
            // 中
            GUI.DrawTextureWithTexCoords(
                new Rect(rect.x + border, rect.y + border, rect.width - 2 * border, rect.height - 2 * border),
                tex,
                new Rect((float)border / w, (float)border / h, 1f - 2f * border / w, 1f - 2f * border / h)
            );
            // 右
            GUI.DrawTextureWithTexCoords(
                new Rect(rect.x + rect.width - border, rect.y + border, border, rect.height - 2 * border),
                tex,
                new Rect(1f - (float)border / w, (float)border / h, (float)border / w, 1f - 2f * border / h)
            );
            // 左下
            GUI.DrawTextureWithTexCoords(
                new Rect(rect.x, rect.y + rect.height - border, border, border),
                tex,
                new Rect(0f, 0f, (float)border / w, (float)border / h)
            );
            // 下
            GUI.DrawTextureWithTexCoords(
                new Rect(rect.x + border, rect.y + rect.height - border, rect.width - 2 * border, border),
                tex,
                new Rect((float)border / w, 0f, 1f - 2f * border / w, (float)border / h)
            );
            // 右下
            GUI.DrawTextureWithTexCoords(
                new Rect(rect.x + rect.width - border, rect.y + rect.height - border, border, border),
                tex,
                new Rect(1f - (float)border / w, 0f, (float)border / w, (float)border / h)
            );
        }
    }
}
