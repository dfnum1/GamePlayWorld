...
namespace Framework.Plugin.Polygon
{
    ...
    public class Polygon2Triangulation
    {
        ...
        /// 判断凹凸的时候的比对轴
        /// </summary>
        private CompareAxle m_compareAxle = CompareAxle.Y;

        /// <summary>
        /// 多边形顶点
        /// </summary>
        private List<Vector3> m_polygonVertexs = new List<Vector3>();
        ...
        public Polygon2Triangulation(List<Vector3> polygonVertexs)
        {
            Set(polygonVertexs);
        }

        public void Set(List<Vector3> polygonVertexs)
        {
            this.m_polygonVertexs = polygonVertexs;
        ...
        {
            this.m_compareAxle = compareAxle;
        }
        ...
        private SplitResult SplitPolygon()
        {
            ...
            for (int i = 0; i < m_nodeManager.LinkedListLength; i++)
            {
                ...
                if (m_compareAxle == CompareAxle.Y)
                {
                    if (crossRes.y > 0)
                    {
                ...
                if (m_compareAxle == CompareAxle.X)
                {
                    if (crossRes.x > 0)
                    {
                        _concaveVertexs.Add(currentNode);
                ...
                if (m_compareAxle == CompareAxle.Z)
                {
                    if (crossRes.z > 0)
                    {
                        _concaveVertexs.Add(currentNode);
     ...
     //       Debug.Log("-----------------------------------------------");
            ...
            m_vertexsSequence.Add(_polygonEars[0].lastNode.id);
            m_vertexsSequence.Add(_polygonEars[0].id);
            m_vertexsSequence.Add(_polygonEars[0].nextNode.id);
            m_nodeManager.RemoveNode(_polygonEars[0]);
