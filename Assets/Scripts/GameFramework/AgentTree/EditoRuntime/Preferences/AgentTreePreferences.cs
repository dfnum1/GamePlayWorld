...
namespace Framework.Plugin.AT
{
    public class AgentTreePreferences
    {
        ...
        public class Settings : ISerializationCallbackReceiver
        {
            ...
            [SerializeField]
            private Color _delegateLinkLineColor = Color.yellow;

            [SerializeField]
            private AgentTreeEditorPath _editorPathConfig = new AgentTreeEditorPath();
            ...
            public AgentTreeEditorPath editorPathConfig { get { return _editorPathConfig; } set { _editorPathConfig = value; } }

            public Color linkLineColor { get { return _linkLineColor; } set { _linkLineColor = value; } }
            public Color delegateLinkLineColor { get { return _delegateLinkLineColor; } set { _delegateLinkLineColor = value; } }
            ...
            public Color32 nodeSearchColor = new Color32(0,255,0, 148);
            public Color32 nodeBgColor = new Color32(255, 255, 255, 148);
            public Color32 nodeRefPortColor = new Color32(128, 128, 128, 50);
            ...
            public Texture2D gridTexture
            {
                get
                {
                    if (_gridTexture == null) _gridTexture = AgentTreeEditorResources.GenerateGridTexture(gridLineColor, gridBgColor);
                    return _gridTexture;
                }
            }
            ...
            public Texture2D crossTexture
            {
                get
                {
                    if (_crossTexture == null) _crossTexture = AgentTreeEditorResources.GenerateCrossTexture(gridLineColor);
...
//#if UNITY_2019_1_OR_NEWER
        ...
        public static SettingsProvider CreateNodeSettingsProvider() {
            SettingsProvider provider = new SettingsProvider("Preferences/GameFramework/ATEditor", SettingsScope.User) {
...
//#endif
        ...
        {
            VerifyLoaded();
            Settings settings = AgentTreePreferences.settings[lastKey];

            ATPathGUI(lastKey, settings);
        ...
        private static void GridSettingsGUI(string key, Settings settings)
        {
            ...
            if (GUI.changed)
            {
                ...
                AgentTreeEditorUtils.RepaintAll();
            }
        ...
        private static void SystemSettingsGUI(string key, Settings settings)
        {
            ...
            if(settings._iconTexture == null)
                settings._iconTexture = Resources.Load<Texture2D>("ATIcon");
            settings._iconTexture = EditorGUILayout.ObjectField("icon", settings._iconTexture, typeof(Texture2D), false) as Texture2D;
        ...
        private static void NodeSettingsGUI(string key, Settings settings)
        {
            ...
            settings.nodeSearchColor = EditorGUILayout.ColorField("Search", settings.nodeSearchColor);
            settings.nodeBgColor = EditorGUILayout.ColorField("NodeBG", settings.nodeBgColor);
            settings.nodeRefPortColor = EditorGUILayout.ColorField("RefPort", settings.nodeRefPortColor);
        ...
        private static void ATPathGUI(string key, Settings settings)
        {
            //Label
            EditorGUILayout.LabelField("PathConfig", EditorStyles.boldLabel);
            settings.editorPathConfig.autoGeneratorBuildPath = EditorGUILayout.TextField("CodeEditorGenerator", settings.editorPathConfig.autoGeneratorBuildPath);
            settings.editorPathConfig.autoRuntimeMonoPath = EditorGUILayout.TextField("CodeRuntimeGenerator", settings.editorPathConfig.autoRuntimeMonoPath);
            if (GUI.changed)
            {
                SavePrefs(key, settings);
                AgentTreeEditorUtils.RepaintAll();
        ...
        private static void TypeColorsGUI(string key, Settings settings)
        {
            ...
            {
                string typeColorKey = AgentTreeEditorUtils.PrettyName(type);
        ...
        public static Color GetTypeColor(System.Type type)
        {
            ...
            {
                string typeName = AgentTreeEditorUtils.PrettyName(type);
