...
namespace Framework.Plugin.AT
{
    ...
    [System.Serializable]
    public class VariableInt : AbsVariable<int>
    {
        public static implicit operator VariableInt(int value) { return new VariableInt { mValue = value }; }
        //------------------------------------------------------
        public override void Init(AgentTree pTree)
        {
            ...
            {
                pBindVariable = pTree.GetVariable<VariableInt>(bindVarGuid);
            }
        }
        //------------------------------------------------------
        public override int GetClassHashCode() { return mValue; }
        public override void SetClassHashCode(int hashCode) { this.mValue = hashCode; }
        ...
        public override int CompareTo(Variable oth)
        {
            ...
            if (oth is VariableInt) return mValue-((VariableInt)oth).mValue;
            return 1;
        }
        //------------------------------------------------------
        ...
        public override void AddTo(Variable oth, Variable refTo = null)
        {
            ...
            if (refTo != null && refTo is VariableInt)
            {
                ...
                ((VariableInt)refTo).mValue = (mValue + ((VariableInt)oth).mValue);
                return;
            }
            if (IsFlag(EFlag.Const)) return;
            mValue += ((VariableInt)oth).mValue;
        ...
        {
            if (!(oth is VariableInt)) return;
            if ((oth as VariableInt).mValue == 0) return;
        ...
        public override void Min(Variable oth, Variable refTo = null)
        {
            ...
            {
                if (refTo.IsFlag(EFlag.Const)) return;
                if (oth is VariableInt) (refTo as VariableInt).mValue = Mathf.Min((oth as VariableInt).mValue, mValue);
                else if (oth is VariableByte) (refTo as VariableInt).mValue = Mathf.Min((oth as VariableByte).mValue, mValue);
            }
            else
            {
                if (IsFlag(EFlag.Const)) return;
        ...
        public override void Max(Variable oth, Variable refTo = null)
        {
            ...
            {
                if (refTo.IsFlag(EFlag.Const)) return;
                if (oth is VariableByte) (refTo as VariableInt).mValue = Mathf.Max((oth as VariableByte).mValue, mValue);
                else if (oth is VariableInt) (refTo as VariableInt).mValue = Mathf.Max((oth as VariableInt).mValue, mValue);
            }
            else
            {
                if (IsFlag(EFlag.Const)) return;
        ...
        public override void Clamp(Variable left, Variable right, Variable refTo = null)
        {
            ...
            {
                if (refTo.IsFlag(EFlag.Const)) return;
                if (left is VariableByte) (refTo as VariableInt).mValue = Mathf.Clamp(mValue, (left as VariableByte).mValue, (right as VariableByte).mValue);
                else if (left is VariableInt) (refTo as VariableInt).mValue = Mathf.Clamp(mValue, (left as VariableInt).mValue, (right as VariableInt).mValue);
            }
            else
            {
                if (IsFlag(EFlag.Const)) return;
        ...
        public override Rect OnGUI(DrawUIParam param)
        {
            ...
            if (ret == null)
                mValue = UnityEditor.EditorGUILayout.IntField(label, mValue);
            else
            {
                mValue = System.Convert.ToInt32(ret);
            }
    ...
    //------------------------------------------------------
    [VariableType(EVariableType.IntList)]
    [System.Serializable]
    public class VariableIntList : ListVariable<List<int>>
    {
        public static implicit operator VariableIntList(List<int> value) { return new VariableIntList { mValue = value }; }
        //------------------------------------------------------
        public override void Init(AgentTree pTree)
        {
            base.Init(pTree);
            if (bindVarGuid != 0 && bindVarGuid != this.GUID)
            {
                pBindVariable = pTree.GetVariable<VariableIntList>(bindVarGuid);
            }
        }
        //------------------------------------------------------
        ...
        public override void Copy(BaseNode pOther, bool bIncludeGuid = true)
        {
            ...
            VariableIntList pVar = pOther as VariableIntList;
            if (pVar.mValue != null && pVar.mValue.Count > 0)
            {
                mValue = new List<int>(pVar.mValue.Count);
        ...
        {
            if (IsFlag(EFlag.Const)) return;
            if (oth is VariableIntList)
                mValue = ((VariableIntList)oth).mValue;
        }
        //------------------------------------------------------
        public override void Clamp(Variable left, Variable right, Variable refTo = null)
        {
            if (left is VariableInt && right is VariableInt)
            {
                if (refTo != null && refTo is VariableIntList)
                {
                    if (refTo.IsFlag(EFlag.Const)) return;
                    VariableIntList temp = refTo as VariableIntList;
                    if (temp.mValue.Count != mValue.Count) return;
                    for (int i = 0; i < mValue.Count; ++i)
                    {
                        temp.mValue[i] = Mathf.Clamp(mValue[i], (left as VariableInt).mValue, (right as VariableInt).mValue);
                    }
                }
                else
                {
                    ...
                    for (int i = 0; i < mValue.Count; ++i)
                    {
                        mValue[i] = Mathf.Clamp(mValue[i], (left as VariableInt).mValue, (right as VariableInt).mValue);
                    }
                }
            }
        }
        ...
        public override void Lerp(Variable oth, float fValue, Variable refTo = null)
        {
            if (!(oth is VariableIntList)) return;
            VariableIntList oth_ = (VariableIntList)oth;
            if (oth_.mValue.Count != mValue.Count) return;
            if (refTo != null)
            {
                if (refTo.IsFlag(EFlag.Const)) return;
                VariableIntList refTo_ = refTo as VariableIntList;
                if (refTo_.mValue.Count != mValue.Count) return;
                for (int i = 0; i < mValue.Count; ++i)
                {
                    refTo_.mValue[i] = (int)Mathf.Lerp(mValue[i], oth_.mValue[i], fValue);
                }
            }
            else
            {
                ...
                for (int i = 0; i < mValue.Count; ++i)
                {
                    mValue[i] = (int)Mathf.Lerp(mValue[i], oth_.mValue[i], fValue);
                }
            }
        }
        //------------------------------------------------------
        public override void AddToList(Variable item)
        {
            ...
            {
                mValue.Add((int)(item as VariableByte).mValue);
            }
        }
        //------------------------------------------------------
        public override int IndexofList(Variable item)
        {
            if (item is VariableInt)
            {
                return mValue.IndexOf((item as VariableInt).mValue);
            }
            return -1;
        }
        //------------------------------------------------------
        ...
        public override int CompareTo(Variable oth)
        {
            if (oth == null || oth.GetType() != typeof(VariableIntList)) return 1;
            VariableIntList othList = oth as VariableIntList;
        ...
        public override Rect OnGUI(DrawUIParam param)
        {
            if (mValue == null) mValue = new List<int>();
            ...
            if (IsFlag(EFlag.Expanded))
            {
                ...
                for (int i = 0; i < mValue.Count; ++i)
                {
                    ...
                    if (ret == null)
                        mValue[i] = UnityEditor.EditorGUILayout.IntField("[" + i + "]", mValue[i]);
                    else
                        mValue[i] = System.Convert.ToInt32(ret);
