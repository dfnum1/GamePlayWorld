...
namespace Framework.Cutscene.Runtime
{
    ...
    public class CutsceneInstance : IUserData
    {
        ...
        private LinkedList<AInstanceAble> m_vInstances = null;
        private Dictionary<string, Asset> m_vAssets = null;
        ...
        {
            return InnerCreate(data);
        }
        //-----------------------------------------------------
        public bool CreateSubByName(CutsceneGraph data, string subName = null)
        {
            return InnerCreate(data, subName);
        }
        //-----------------------------------------------------
        public bool CreateSubById(CutsceneGraph data, int subId = -1)
        {
            return InnerCreate(data, null, subId);
        }
        //-----------------------------------------------------
        internal bool InnerCreate(CutsceneGraph data, string subName = null, int subId = -1)
        {
            Clear();
            if (data == null)
            {
            ...
            m_pData = data;
            //! create playable
            CutsceneData playData = null;
            if (!string.IsNullOrEmpty(subName)) playData = data.GetSubCutscene(subName);
            else if (subId >= 0 && subId < ushort.MaxValue) playData = data.GetSubCutscene((ushort)subId);
            else playData = data.GetEnterCutscene();
            CreatePlayable(playData);
        ...
        //-----------------------------------------------------
        public bool Create(Asset pAsset, string subName = null, int subId = -1)
        {
            Clear();
            if (pAsset == null)
                return false;
            var cutsceneObj = pAsset.GetOrigin<CutsceneObject>();
            if (cutsceneObj == null)
            {
                Debug.LogError("CutsceneInstance Create failed, asset is not a CutsceneObject: " + pAsset.Path);
                return false;
            }
            m_pAsset = pAsset;
            pAsset.Grab();

            m_pData = cutsceneObj.GetCutsceneGraph();
            //! create playable
            CutsceneData playData = null;
            if (!string.IsNullOrEmpty(subName)) playData = m_pData.GetSubCutscene(subName);
            else if (subId >= 0 && subId < ushort.MaxValue) playData = m_pData.GetSubCutscene((ushort)subId);
            else playData = m_pData.GetEnterCutscene();
            CreatePlayable(playData);

            //! create agent tree
            CreateAgentTree(m_pData);
        ...
        //-----------------------------------------------------
        public AssetOperiaon LoadAsset(string file, bool bAsync = true, IUserData pOwner = null)
        {
            if (m_pMgr == null)
            {
                return null;
            }
            AssetOperiaon pOp = m_pMgr.LoadAsset(this, file, bAsync);
            if (pOp == null)
                return null;
            pOp.OnCallback += OnLoadAsset;
            pOp.userData = pOwner;
            return pOp;
        }
        //-----------------------------------------------------
        void OnLoadAsset(AssetOperiaon assetOp)
        {
            ...
            if (m_vAssets == null)
                m_vAssets = new Dictionary<string, Asset>(4);
            m_vAssets[assetOp.strFile] = assetOp.pAsset;

            if(assetOp.userData!=null && assetOp.userData is ACutsceneDriver)
            {
                ((ACutsceneDriver)assetOp.userData).OnLoadAsset(assetOp.pAsset);
            }
        }
        //-----------------------------------------------------
        public InstanceOperiaon SpawnInstance(string file, bool bAsync = true, IUserData pOwner = null)
        {
            if (m_pMgr == null)
            {
                return null;
            }
            InstanceOperiaon pInstanceOp = m_pMgr.SpawnInstance(this, file, bAsync);
            if (pInstanceOp == null)
                return null;
            pInstanceOp.OnCallback += OnSpawnInstance;
            pInstanceOp.OnSign += OnSpawnSign;
            pInstanceOp.userData0 = pOwner;
            return pInstanceOp;
        }
        //-----------------------------------------------------
        internal void OnSpawnInstance(InstanceOperiaon pOperaion)
        {
            if(pOperaion.pPoolAble == null)
            {
                return;
            }
            if (m_vInstances == null)
                m_vInstances = new LinkedList<AInstanceAble>();
            m_vInstances.AddLast(pOperaion.pPoolAble);

            if(pOperaion.userData0!=null && pOperaion.userData0 is ACutsceneDriver)
            {
                ((ACutsceneDriver)pOperaion.userData0).OnSpawnInstance(pOperaion.pPoolAble);
            }
#if UNITY_EDITOR
            if (m_pOwnerEditor != null && m_pOwnerEditor is Editor.ACutsceneEditor)
            {
                ((Editor.ACutsceneEditor)m_pOwnerEditor).OnSpawnInstance(pOperaion.pPoolAble);
            }
#endif
        }
        //-----------------------------------------------------
        internal void OnSpawnSign(InstanceOperiaon pOperaion)
        {
            pOperaion.bUsed = !IsDestroyed();
            if(pOperaion.bUsed)
            {
                if(pOperaion.userData0!=null && pOperaion.userData0 is ACutsceneDriver)
                {
                    pOperaion.bUsed = ((ACutsceneDriver)pOperaion.userData0).IsValid();
                }
            }
        }
        //-----------------------------------------------------
        public void DespawnInstance(AInstanceAble pInstance)
        {
            ...
            m_pMgr.DespawnInstance(pInstance);
        }
        //-----------------------------------------------------
        ...
        //-----------------------------------------------------
        public void BindData(IUserData pData)
        {
            m_BindData = pData;
        }
        //-----------------------------------------------------
        public IUserData GetBindData()
        {
            return m_BindData;
        }
        //-----------------------------------------------------
