...
namespace Framework.Cutscene.Runtime
{
    ...
    public class ParticleClip : IBaseClip
    {
        ...
        [Display("识别ID"), StateGUIByField("bindTrack", "true")] public int objId;
		[Display("轴同步")] public bool syncAxis = true; // 是否同步轴向
		 [Display("速度"), StateGUIByField("syncAxis", "true")] public float playSpeed = 1.0f; 
        //-----------------------------------------------------
    ...
    public class ParticleDriver : ACutsceneDriver, ICutsceneObject
    {
        ...
        //-----------------------------------------------------
        public AInstanceAble GetInstance()
        {
            return m_Instance;
        }
        //-----------------------------------------------------
        ...
        public override void OnSpawnInstance(AInstanceAble pInstance)
        {
            ...
            if (m_Instance == pInstance)
                return;
            // 实例化特效
            m_Instance = pInstance;

			UpdateTransform();
			if(m_bSyncAxis || IsEditorMode())
            {
        ...
        void CheckBindTransform(CutsceneTrack pTrack, ParticleClip parClip)
        {
            ...
            foreach (var db in vObjs)
            {
                ...
                {
                    var node = go.transform.Find(parClip.bindNode);
                    if (node != null)
                        m_BindTrans = node;
                    else
        ...
        public override bool OnClipEnter(CutsceneTrack pTrack, FrameData clip)
        {
            ...
            SpawnInstance(parClip.prefabName, true);
