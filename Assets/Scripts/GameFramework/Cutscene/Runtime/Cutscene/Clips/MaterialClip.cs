...
namespace Framework.Cutscene.Runtime
{
    ...
    public class MaterialClip : IBaseClip
    {
        ...
        [Display("变量名"), DisableGUI] public string              propName = "_Color";
        [Display("变量名"), DisableGUI] public ShaderPropertyType propType = ShaderPropertyType.Vector;
        [Display("原属性"), DisableGUI] public Vector4             propValue = new Vector4(1, 1, 1, 1);
        [Display("目标属性"), DisableGUI] public Vector4           toPropValue = new Vector4(1, 1, 1, 1);
        [Display("贴图"), StringViewPlugin("OnDrawSelectTextureInspector"), DisableGUI]
        public string                 textureName = null;
        [Display("控制曲线"), DisableGUI] public AnimationCurve     propCurve;
        //-----------------------------------------------------
    ...
    public class MaterialDriver : ACutsceneDriver
    {
        ...
        void LoadTexture(string texture)
        {
            ...
            if(texture.CompareTo(m_curTextureName) !=0)
            {
                ClearTexture();
            }
            LoadAsset(texture);
            m_curTextureName = texture;
        }
        //-----------------------------------------------------
        public override void OnLoadAsset(Asset pAsset)
        {
            if (pAsset == null)
                return;
            m_pTexture = pAsset.GetOrigin<Texture>();
        }
        //-----------------------------------------------------
        ...
        public override bool OnFrameClip(CutsceneTrack pTrack, FrameData frameData)
        {
            ...
            switch (propType)
            {
                ...
                case UnityEngine.Rendering.ShaderPropertyType.Texture:
                    {
                        // 贴图类型只在进入时设置，不做插值
                        if(!m_bSetTexture && m_pTexture!=null)
                        {
                            m_pMaterial.SetTexture(clipData.propName, m_pTexture);
                        }
                    }
                    break;
            }
