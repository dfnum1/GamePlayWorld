    ...
    public class ControllerRefUtil
    {
        ...
        public static bool IsEditorObject(UnityEngine.Object pObject)
        {
            if (pObject == null)
                return false;
#if USE_CUTSCENE
            if (Editor.CameraPathEditor.IsEditing() && pObject is Camera)
            {
            ...
            if (pTransform == null)
                return false;
#if USE_CUTSCENE
            if (Editor.CurvePathEditor.IsEditing(pTransform) && pTransform == Selection.activeTransform)
                return true;
#endif
            return false;
        }
        ...
        public static bool IsEditorObject(ICutsceneObject pObject)
        {
            ...
            if(ms_pEditingObject == pObject)
                return true;
#if USE_CUTSCENE
            if (Editor.CameraPathEditor.IsEditing() && pObject.GetCamera())
            {
            ...
            if (ms_CameraEditing != null && ms_CameraEditing == pObject.GetCamera())
                return true;
#if USE_CUTSCENE
            if (Editor.CurvePathEditor.IsEditing(pObject))
                return true;
#endif
            ...
            if (pObj == null)
                return false;
#if USE_CUTSCENE
            if (Editor.CurvePathEditor.IsEditing(pObj) && pObj == Selection.activeTransform)
                return true;
#endif
            return false;
        }
