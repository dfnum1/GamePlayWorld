...
{
    [System.Serializable, CutsceneEvent("执行行为树节点")]
    public class CutsceneExecuteAT : IBaseEvent
    {
        [System.Serializable]
        public struct Port
        {
        ...
        {
            return new CutsceneExecuteATNodeDriver();
        }
        //-----------------------------------------------------
        public ushort GetIdType()
        {
            return (ushort)EEventType.eExecuteATNode;
        }
        //-----------------------------------------------------
    ...
    //-----------------------------------------------------
    internal class CutsceneExecuteATNodeDriver : ACutsceneDriver
    {
        public override bool OnEventTrigger(CutsceneTrack pTrack, IBaseEvent pEvt)
        {
            var portData = pEvt.Cast<CutsceneExecuteAT>();
            ...
            if (ports != null && portData.paramPorts!=null)
            {
