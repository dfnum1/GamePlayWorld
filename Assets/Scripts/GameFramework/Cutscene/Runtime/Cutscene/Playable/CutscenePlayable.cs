...
namespace Framework.Cutscene.Runtime
{
    ...
    public class CutscenePlayable
    {
        ...
        public void Stop(bool bTimeOver = false)
        {
            ...
            if (bTimeOver)
            {
                ...
                if (m_vTracks != null)
                {
                    ...
                    {
                        CutsceneTrack track = m_vTracks[i];
                        track.StopTrigger(ref m_pFrameData);
                    }
                }
            }
        ...
        {
            if (m_pCutscene != null) m_pCutscene.BindData(pData);
        }
        //-----------------------------------------------------
        public IUserData GetBindData()
        {
            if (m_pCutscene != null) return m_pCutscene.GetBindData();
            return m_pCutscene.GetBindData();
        }
        //-----------------------------------------------------
        ...
        public void Update(float fDelta)
        {
            ...
            if (fDelta > 0.0f)
            {
                ...
                while (m_fAccumoulator >= m_fInvFrameRate && m_eStatus == EPlayableStatus.Start)
                {
