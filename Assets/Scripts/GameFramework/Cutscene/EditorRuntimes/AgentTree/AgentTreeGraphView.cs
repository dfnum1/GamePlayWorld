    ...
    public class AgentTreeGraphView : GraphView , IAgentTreeCallback
    {
        ...
        private bool OnMenuSelectEntry(SearchTreeEntry searchTreeEntry, SearchWindowContext context)
        {
            ...
            if (searchTreeEntry.userData is AgentTreeAttri)
            {
                ...
                {
                    CutsceneAT.Runtime.ActionNode pAction = new CutsceneAT.Runtime.ActionNode();
                    pAction.type = attri.actionType;
                    pAction.guid = GeneratorGUID();
                    pAction.nextActions = null;
                    newNode = pAction;
        ...
        //--------------------------------------------------------
        public void RemoveNode(CutsceneAT.Runtime.BaseNode pNode)
        {
            if(m_vNodes.TryGetValue(pNode, out var graphNode))
            {
                graphNode.Release();
        ...
        public void Save(AgentTreeData pData)
        {
            ...
            List<CutsceneAT.Runtime.ActionNode> vActions = new List<CutsceneAT.Runtime.ActionNode>();
            ...
            foreach (var db in m_vNodes)
            {
                ...
                else if (graphNode.bindNode is CutsceneAT.Runtime.ActionNode)
                {
                    vActions.Add(graphNode.bindNode as CutsceneAT.Runtime.ActionNode);
                }
            }
        ...
        //-----------------------------------------------------
        public void OnNotifyCutsceneATExecutedNode(AgentTree pAgentTree, BaseNode pNode)
        {
