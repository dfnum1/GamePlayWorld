...
namespace Framework.Cutscene.Editor
{
    ...
    public class InspectorDrawLogic : ACutsceneLogic, UndoHandler
    {
        ...
        //--------------------------------------------------------
        public override void OnPreviewDraw(int controllerId, Camera camera, Event evt)
        {
            if (m_vSelectClips == null || m_vSelectClips.Count <= 0)
                return;
            for (int i = 0; i < m_vSelectClips.Count; i++)
            {
                var draw = m_vSelectClips[i];
                if (draw == null) continue;
                draw.GetData().OnSceneView(SceneView.lastActiveSceneView, this);
            }
        }
        //--------------------------------------------------------
        protected override void OnGUI()
        {
            ...
            try
            {
                if (m_pSelectGroup != null)
                {
                    ...
                    obj = (CutsceneObjectBinder)EditorGUILayout.ObjectField("绑定对象", obj, typeof(CutsceneObjectBinder), true);
