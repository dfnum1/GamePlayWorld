...
#region 定义结构
        ...
        [Serializable]
        public class ScrollRenderEvent : UnityEvent<int, UISerialized> { }
        ...
        private class ItemInfo
        {
            ...
            public Framework.UI.UISerialized serialized;
            ...
            public ItemInfo(RectTransform trans)
            {
                ...
                if(transform)
                {
                    serialized = transform.GetComponent<Framework.UI.UISerialized>();
                    if (serialized == null) serialized = transform.gameObject.AddComponent<Framework.UI.UISerialized>();
                }
...
#region 属性
        ...
        public void InitAndShow(uint count, UnityAction<int,UISerialized> evt)
        {
            onItemRender.RemoveAllListeners();
            onItemRender.AddListener(evt);
            numItems = count;
        }
...
#region 列表重载
        ...
        void OnInitializePotentialDragItem(GameObject go, BaseEventData eventData, params IUserData[] param)
        {
        ...
        void OnBeginDragItem(GameObject go, BaseEventData eventData, params IUserData[] param)
        {
        ...
        void OnEndDragItem(GameObject go, BaseEventData eventData, params IUserData[] param)
        {
        ...
        void OnDragItem(GameObject go, BaseEventData eventData, params IUserData[] param)
        {
        ...
        protected void LateUpdate()
        {
            ...
            if (m_EnterScale)
            {
                if(m_vEnterScale!=null)
                {
                    ...
                    EnterScale enter;
                    float curveTime = BaseUtil.GetCurveMaxTime(m_EnterScaleAnimation);
...
#region 渲染
        ...
        private IEnumerator RebuildList()
        {
            if (!Application.isPlaying || !m_HasInit || !m_Content || !m_Prefab || !m_ViewPort)
                yield return null;
            yield return BaseUtil.waitForEndOfFrame;
        ...
        public void RefreshList()
        {
            ...
            //   UpdatePosition(true);
            //    BeginCoroutine(InnerRefreshList());
        }
        ...
        int GetAutoAttachIndex()
        {
            ...
            if (Math.Abs(itemSizeAndSpace.y) <= 0.001f)
                itemSizeAndSpace.y = 1;
            if (Math.Abs(itemSizeAndSpace.x) <= 0.001f)
                itemSizeAndSpace.x = 1;
        ...
        public UISerialized GetItem(int actualIndex)
        {
            if (actualIndex < 0)
            {
                return null;
