...
类    名: 	PlayableGraphVisualizerWindow
作    者:	HappLI
描    述:	
*********************************************************************/
...
namespace Framework.GraphVisualizer
{
    public class PlayableGraphVisualizerWindow : EditorWindow, IHasCustomMenu
    {
        ...
        static Framework.Core.IUserPlayableExternDraw ms_CurrentExternDraw;
...
#region Configuration
        ...
        public static void ShowWindow(PlayableGraph playableGraph, Framework.Core.IUserPlayableExternDraw externDraw = null)
        {
            PlayableGraphVisualizerWindow window = GetWindow<PlayableGraphVisualizerWindow>("PlayableGraph Visualizer");
            window.m_CurrentGraph = playableGraph;
            ms_CurrentExternDraw = externDraw;
        }

        internal static void ExternDrawInfo(ref System.Text.StringBuilder sb)
        {
            if(ms_CurrentExternDraw!=null)
            {
                ms_CurrentExternDraw.OnDrawInfo(ref sb);
            }
        }
        ...
        void OnGUI()
        {
            ...
            GUILayout.BeginVertical();
            int currentSelection = selectedGraphs.IndexOf(m_CurrentGraph);
            m_CurrentGraph = GetSelectedGraphInToolBar(selectedGraphs, m_CurrentGraph);
            if (currentSelection != selectedGraphs.IndexOf(m_CurrentGraph))
                ms_CurrentExternDraw = null;
        ...
        private List<PlayableGraph> GetGraphList()
        {
            ...
            foreach (var clientGraph in Framework.Core.GraphVisualizerClient.GetGraphs())
            {
