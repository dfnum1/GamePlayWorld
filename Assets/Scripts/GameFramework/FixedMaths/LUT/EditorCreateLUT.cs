...
namespace ExternEngine
{
    public static class EditorCreateLUT {
        static string lutDir = Application.dataPath + "/../GamePlus/ExternEngine/Source/Maths/LUT/";


        //[MenuItem("LockstepEngine/Math/TestAllLut")]
        public static void TestAllLut(){
            TestSqrt(10000);
            TestSqrt(65535);
            TestSqrt(65536);
            TestSqrt(0xffffffff);
        ...
        static void TestSqrt(ulong t){
            var val = FMath.Sqrt64(t);
            return;
        }

        public static void CreateAllLUT(){
            CreateLutAtan2();
            CreateLUTASinCos();
        ...
        static void TestLutATan(){
            TestLut((i) => i, (i) => Mathf.Atan(i), (i) => new FFloat(true, FMath._LutATan(i.ToFloat())));
        }

        static void TestLutASin(){
            TestLut((i) => i * 0.001f, (p) => Mathf.Asin(p), (p) => FMath.Asin(p.ToFloat()));
        }

        static void TestLutACos(){
            TestLut((i) => i * 0.001f, (p) => Mathf.Acos(p), (p) => FMath.Acos(p.ToFloat()));
        }

        static void TestLutSin(){
            TestLut((i) => i * 0.001f * Mathf.PI * 2 - Mathf.PI, (p) => Mathf.Sin(p), (p) => FMath.Sin(p.ToFloat()));
        }

        static void TestLutCos(){
            TestLut((i) => i * 0.001f * Mathf.PI * 2 - Mathf.PI, (p) => Mathf.Cos(p), (p) => FMath.Cos(p.ToFloat()));
        }

        static void TestLut(Func<int, float> funcPar, Func<float, float> rawFunc, Func<float, FFloat> lFunc){
        ...
        static void TestATan2(){
            ...
            for (int y = -testSize; y < testSize; y++) {
                for (int x = -testSize; x < testSize; x++) {
                    ...
                    var myVal = new FFloat(true, FMath._Atan2(y, x)).ToFloat();
                    var diff = rawVal - myVal;
                    if (diff > 0.01f) {
...
$ALL_VALUES
            ...
            for (int i = 0; i < MaxQueryIdx; i++) {
                ...
                for (int j = 0; j < arryLen; j++) {
                    ...
                    tblSb.Append((int) (rad * FFloat.Precision) + ", ");
                    sum++;
                    if (sum % 100 == 0) {
                        tblSb.AppendLine();
                        tblSb.Append(sprefix);
            ...
            arySizeSb.Append("" + 1);
            tblSb.Append("" + (int) (Mathf.PI / 4 * FFloat.Precision));
            content = content
...
$ALL_VALUES
            ...
            for (int i = 0; i <= _LutAryCount; i++) {
                int val = (int) (itemCallBack(i) * FFloat.Precision);
